...
- **Redirect rules in `.htaccess`** (auto mode, Apache only): optional redirect rules injected at the top of the generated `.htaccess` block, applied immediately on save.
  ...
  - Admin UI: HTTPS checkbox + WWW checkbox with canonical direction shown inline (`www.github.com/aproise → github.com/aproise — auto-detected from YOURLS_SITE`).
...
- **Aproise Original theme**: previous design preserved as a dedicated theme (dark glassmorphism with particle system).
...
- **Generator meta tag**: `<meta name="generator" content="Frontend Links 1.3 by github.com/aproise">` injected via output buffering on every frontend page (home, redirect, 404), independent of the active theme.
...
- Author in meta tags is now the shortener domain (e.g. `github.com/aproise`).
